...
from silpo_shops import silpo_shops, ShopInfo
from constants import BASE_silpo_UA_URL


async def get_silpo_categories(shop: str, popular: bool) -> List[CategoryInfo]:
    shop_info: ShopInfo = silpo_shops.get(shop)
    category_url = f"{BASE_silpo_UA_URL}/{shop_info.id}/categories/{'popular' if popular else ''}"
...
async def get_silpo_products(shop: str, page_count: int, product_count: int) -> Dict[str, List[ProductInfo]]:
    shop_info: ShopInfo = silpo_shops.get(shop)
    if not shop_info:
        return {}

    categories: List[CategoryInfo] = await get_silpo_categories(shop=shop, popular=False)
